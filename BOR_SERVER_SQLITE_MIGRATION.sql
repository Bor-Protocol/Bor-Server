...
-- Check if columns exist first (SQLite doesn't have IF NOT EXISTS for columns)

-- Add user_id to link comments to Protocol accounts
...
-- =====================================================
-- STEP 2: ENHANCE EXISTING AIRESPONSE TABLE
-- =====================================================

-- Add context fields to AIResponse table
ALTER TABLE AIResponse ADD COLUMN responding_to_user_id TEXT; -- Which user triggered this response
ALTER TABLE AIResponse ADD COLUMN session_context TEXT; -- 'public' or appointment_id for private
ALTER TABLE AIResponse ADD COLUMN response_time_ms INTEGER DEFAULT 0; -- How long to generate response
ALTER TABLE AIResponse ADD COLUMN token_count INTEGER DEFAULT 0; -- Tokens used in response
ALTER TABLE AIResponse ADD COLUMN audio_url TEXT; -- Generated TTS audio URL
ALTER TABLE AIResponse ADD COLUMN animation_triggered TEXT; -- Animation that was triggered

-- Add indexes for AIResponse
CREATE INDEX IF NOT EXISTS idx_airesponse_user ON AIResponse(responding_to_user_id);
CREATE INDEX IF NOT EXISTS idx_airesponse_session ON AIResponse(session_context);
CREATE INDEX IF NOT EXISTS idx_airesponse_agent_date ON AIResponse(agentId, createdAt DESC);
...
SELECT 
    ...
    (SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name IN (
        ...
        'moderation_logs', 'system_health', 'session_analytics'
    )) as new_tables_created,
    (SELECT COUNT(*) FROM Comments) as total_comments,
    (SELECT COUNT(*) FROM AIResponse) as total_ai_responses,
...
SELECT 
    'ENHANCED AIRESPONSE TABLE' as info,
...
SELECT 
    ...
    '1. Integrate JWT middleware with bor-server Express app' as step_1,
    '2. Update Socket.io handlers to use authentication' as step_2,
    '3. Apply rate limiting to API endpoints' as step_3,
    '4. Start performance monitoring' as step_4,
    '5. Begin frontend authentication integration' as step_5;